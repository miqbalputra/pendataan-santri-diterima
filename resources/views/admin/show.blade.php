            ...
            <div>
                <nav class="flex mb-2" aria-label="Breadcrumb">
                    <ol class="inline-flex items-center space-x-1 md:space-x-3 text-xs font-medium text-slate-500">
                        ...
                        <li class="text-slate-800">Detail Peserta Didik</li>
                    </ol>
                </nav>
                ...
                <p class="text-slate-500 mt-1">Nomor Pendataan: {{ $santri->nomor_pendaftaran ?? ('SPSB-' . $santri->created_at->format('Y') . '-' . str_pad($santri->id, 5, '0', STR_PAD_LEFT)) }} | Data masuk pada: {{ $santri->created_at->format('d M Y, H:i') }}</p>
            </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div class="space-y-8">
                ...
                <div class="glass-card rounded-2xl shadow-lg p-6 border-t-4 border-emerald-500 sticky top-6">
                    ...
                    <form action="{{ route('admin.status', $santri->id) }}" method="POST" class="space-y-4">
                        ...
                        <div>
                            ...
                            <select name="status_pendaftaran" class="w-full border-slate-200 rounded-xl py-3 px-4 bg-slate-50 font-bold text-slate-700 focus:ring-emerald-500 focus:border-emerald-500 shadow-inner">
                                <option value="Pending" {{ $santri->status_pendaftaran == 'Pending' ? 'selected' : '' }}>MENUNGGU VERIFIKASI DATA</option>
                                <option value="Diterima" {{ $santri->status_pendaftaran == 'Diterima' ? 'selected' : '' }}>DATA LENGKAP / TERVERIFIKASI</option>
                                <option value="Ditolak" {{ $santri->status_pendaftaran == 'Ditolak' ? 'selected' : '' }}>DATA TIDAK VALID / TIDAK DILANJUTKAN</option>
                            </select>
                        </div>
                    ...
                    <div class="mt-6 pt-6 border-t border-slate-100 flex flex-col gap-3">
                        @php
                            ...
                            $waText = urlencode("Assalamu'alaikum Bapak/Ibu {$santri->nama_ayah}, kami dari Panitia SPSB Griya Qur'an.\n\nTerkait pendataan ananda *{$santri->nama_lengkap}*, [Sebutkan informasi].");
                        @endphp
                        ...
                        <a href="{{ \Illuminate\Support\Facades\URL::signedRoute('pendaftaran.cetak', ['id' => $santri->id]) }}" class="w-full bg-slate-100 hover:bg-slate-200 text-slate-700 font-bold py-3 px-4 rounded-xl transition flex items-center justify-center gap-2 text-sm">
                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 17h2a2 2 0 002-2v-4a2 2 0 00-2-2H5a2 2 0 00-2 2v4a2 2 0 002 2h2m2 4h6a2 2 0 002-2v-4a2 2 0 00-2-2H9a2 2 0 00-2 2v4a2 2 0 002 2zm8-12V5a2 2 0 00-2-2H9a2 2 0 00-2 2v4h10z"></path></svg>
                            Cetak Formulir (PDF)
                        </a>
                        <a href="{{ \Illuminate\Support\Facades\URL::signedRoute('pendaftaran.bukti', ['id' => $santri->id]) }}" target="_blank" class="w-full bg-emerald-50 hover:bg-emerald-100 text-emerald-700 font-bold py-3 px-4 rounded-xl transition flex items-center justify-center gap-2 text-sm border border-emerald-100">
                            Bukti Pendataan + QR
                        </a>
                ...
                <div class="glass-card rounded-2xl shadow-sm p-6">
                    ...
                    <p class="text-xs font-semibold text-slate-500 mb-4 leading-relaxed">Pilih <span class="font-extrabold text-rose-600">Perlu Perbaikan</span> hanya pada dokumen yang harus diunggah ulang oleh orang tua. Di halaman revisi, hanya dokumen tersebut yang akan ditampilkan sebagai prioritas.</p>
                    <div class="space-y-4">
                        @php
                        ...
                        <div class="border border-slate-100 rounded-xl p-3 bg-slate-50 group transition hover:bg-white hover:shadow-md">
                            ...
                            <select name="dokumen_status[{{ $doc['field'] }}]" class="mb-3 w-full rounded-xl border-slate-200 bg-white px-3 py-2 text-xs font-bold text-slate-700">
                                @foreach([
                                    'menunggu_review' => 'Menunggu Verifikasi',
                                    'valid' => 'Valid',
